...
class DatabaseService {
  ...
  async populateDb (collectionName, data) {
    const itemsCollection = await this.db.collection(collectionName);
    const courses = await itemsCollection.findOne();
    if(courses == null){
      await itemsCollection.insertMany(data)
      .then(result => console.log(`Successfully inserted item with _id: ${result.insertedId}`))
      .catch(err => console.error(`Failed to insert item: ${err}`))
    }
    else {
      console.log(`items already inserted`);
    }
  }
