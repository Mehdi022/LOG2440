...
class PlaylistService {
  ...
  async getPlaylistById (id) {
    const res = await this.collection.findOne({ id });
    return res;
  }
  ...
  async updatePlaylist (playlist) {
    ...
    const newPlaylistToAdd = { "$set": playlist};
    await this.collection.updateOne(filter, newPlaylistToAdd);
    await this.savePlaylistThumbnail(playlist);
  }
  ...
  async search (substring, exact) {
    if(exact){
      ...
      const filterDescriptionSensitive = { description: { $regex: `${substring}`} };
      const playlists = await this.collection.find({$or:[filterNameSensitive,filterDescriptionSensitive]}).toArray();
      return playlists;
    }
    const filterNameSensitive = { name: { $regex: `${substring}`, $options: "i" } };
    const filterDescriptionSensitive = { description: { $regex: `${substring}`, $options: "i" } };
    const playlists = await this.collection.find({$or:[filterNameSensitive,filterDescriptionSensitive]}).toArray();
    return playlists;
