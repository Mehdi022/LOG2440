...
class PlaylistManager {
  ...
  async getPlaylistById (id) {
    ...
    const playlistToGet = allPlaylists.find(playlist => playlist.id === id);
    if (playlistToGet) {
  ...
  async updatePlaylist (newPlaylist) {
    ...
    const index = playlists.findIndex((playlist) => playlist.id === newPlaylist.id);
    for (const element in newPlaylist) {
      playlists[index][element] = newPlaylist[element];
    }
    await this.savePlaylistThumbnail(newPlaylist);
    await this.fileSystemManager.writeToJsonFile(this.JSON_PATH, JSON.stringify({ playlists }));
  }
