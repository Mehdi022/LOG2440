...
export class Library {
  ...
  searchInFields (searchFields, searchValue, exactMatch) {
    for (let field of searchFields) {
      if (this.includesSubstring(field, searchValue, exactMatch)) {
        return true;
      }
    }
    return false;
  ...
  search (searchInput, searchSources, exactMatch) {
    let listPlaylists = [];
    for (let playlist of searchSources.playlists){
      if (this.searchInFields(playlist, searchInput.value, exactMatch)) {
        listPlaylists.push(playlist);
      }
    }
    let listSongs = [];
    for (let song of searchSources.songs){
      if (this.searchInFields(song, searchInput.value, exactMatch)) {
        listSongs.push(song);
      }
    }
    this.generateLists(listPlaylists, listSongs);
  }
}
