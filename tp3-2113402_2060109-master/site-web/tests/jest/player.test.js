...
describe.only("Player tests", () => {
  ...
  it("getSongFromIndex should get song for a valid index", () => {
    // TODO
    player.songsInPlayList = songStubs;
    for (let index = 0; index < songStubs.length; index++) {
      const songIndex = player.getSongFromIndex(index);
      expect(songIndex).toEqual(songStubs[index]);
    }
  });
  ...
  it("playPreviousSong should decrement currentIndex when shuffle is disabled", () => {
    ...
    player.loadSongs(songStubs);
    player.songsInPlayList = songStubs;
    for (let index = songStubs.length; index > 0; index--){
      player.currentIndex = index
      player.playPreviousSong();
      expect(player.currentIndex).toEqual(index-1);
    }
  });
  ...
  it("playNextSong should increment currentIndex when shuffle is disabled", () => {
    ...
    player.loadSongs(songStubs);
    player.songsInPlayList = songStubs;
    for (let index = 0; index < songStubs.length - 1; index++){
      player.currentIndex = index
      player.playNextSong();
      expect(player.currentIndex).toEqual(index+1);
    }
  ...
  it("playNextSong should return currentIndex % playlist's length if not shuffled", () => {
    ...
    player.loadSongs(songStubs);
    player.playNextSong();
    expect(player.currentIndex).toEqual(player.currentIndex % player.songsInPlayList.length);
  });
